$(document).ready(function(){
	...
	function getData (start,classify) {
		$.ajax({
			type:"get"
			,url:"http://localhost:8080/tbsupermarket/GetProducts"
			,async:true
			,data:{
				'start':start,
				'classify':classify
			...
			,success:function(data){
				...
				if (data) {
			 		for (var i=0;i<data.length;i++) {
						$('.list').eq(num).append("<li id="+data[i].pro_id+"><a href='javascript:;'><img src="+img_url+data[i].imgUrl+" class='list_img' /><div class='list_main'><p class='list_title'>"+data[i].pro_name+"</p><div class='list_info'><p class='list_sales'>月销"+data[i].count+"件</p><p class='list_price'>￥"+data[i].price+"</p><button class='icon_btn'><span class='icon-cart'></span></button></div></div></a></li>");
					}
...
//		var top=$('.cart').offset().top;//距离文档顶部的距离
		...
		if (username) {
			$.ajax({
				type:"get"
				,url:"http://localhost:8080/tbsupermarket/my_commodity_car.php"
				,async:true
				,data:{
					'user_phone':username,
